...
const buttonStyle = tv({
    ...
    variants: {
        variant: {
            ...
            danger: 'bg-danger text-danger-fg shadow-xs hover:bg-danger/90 focus-visible:ring-danger/25',
        ...
        size: {
            xs: 'gap-x-1.5 px-2 text-xs [--height:theme(spacing.6)] **:[svg]:size-3.5',
            sm: 'gap-x-1.5 px-3 text-sm [--height:theme(spacing.8)] **:[svg]:size-4',
            md: 'gap-x-2 px-4 text-sm [--height:theme(spacing.9)] **:[svg]:size-4',
            lg: 'gap-x-2.5 px-5 text-base [--height:theme(spacing.10)] **:[svg]:size-5'
        },
        icon: {
...
const Button = ({ className, children, variant, size, icon, isPending, ...props }: ButtonProps) => (
    <RACButton
        className={composeRenderProps(className, (className, renderProps) =>
            buttonStyle({ ...renderProps, variant, size, icon, isPending, className })
        )}
        {...props}
    >
        {(values) => (
